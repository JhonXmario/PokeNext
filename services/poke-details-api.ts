...
export async function getPokemonSpecies(url: string): Promise<PokemonSpecies | null> {
  try {
...
export async function getCompletePokemonData(name: string) {
  try {
    ...
    console.error("Error capturando datos completos del Pokémon (valio madres):", error)
    return null
  }
}
