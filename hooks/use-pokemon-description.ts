...
export const usePokemonDescription = (species: PokemonSpecies | null) => {
  const getDescription = () => {
    ...
    const description = spanishEntry?.flavor_text || englishEntry?.flavor_text || "Descripción no disponible"
  ...
  const getGenus = () => {
    ...
    const spanishGenus = species.genera.find((genus) => genus.language.name === "es")
