// components/pokemon/pokemon-data/evolution/hooks/useEvolutionChain.ts
...
export const useEvolutionChain = (evolutionChain: EvolutionChain | null, currentPokemon: string) => {
  ...
  useEffect(() => {
    const fetchEvolutionStages = async () => {
      ...
      try {
        const stages = await parseEvolutionChain(evolutionChain, currentPokemon)
...
const parseEvolutionChain = async (chain: EvolutionChain, currentPokemon: string): Promise<EvolutionStage[]> => {
